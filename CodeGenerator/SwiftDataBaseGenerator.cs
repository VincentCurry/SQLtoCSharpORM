...
namespace CodeGenerator
{
  ...
  classText.AppendLine($"\tcase OpenDatabase(message: String)");
            ...
            foreach (SQLTable table in _sQLTables)
            {
                ...
                classText.AppendLine("");
                GenerateSelectByQueryString(classText, table);
            }
        ...
        private void GenerateInsertFunction(StringBuilder classText, SQLTable table)
        {
            ...
            classText.AppendLine("");
            classText.AppendLine(string.Join("\n", table.Columns.Where(co => co.Nullable).Select(co => $"\t\tappendNonNullValueToColumnAndValue(nullableValue: {Library.LowerFirstCharacter(table.Name)}.{Library.LowerFirstCharacter(co.Name)}, columnName: \"{co.Name}\", columns: &columnSql, values: &valuesSql)")));
            classText.AppendLine("");
            classText.AppendLine($"\t\tlet insertStatement = try prepareStatement(sql: (insertSql + columnSql + valueStatementSql + valuesSql + endSql))");
            classText.AppendLine($"\t\tdefer {{");
            ...
            int columnCounter = 1;
            foreach (SQLTableColumn column in table.Columns.Where(co => !co.Nullable))
            {
        ...
        private void GenerateSelectAllFunction(StringBuilder classText, SQLTable table)
        {
            ...
            classText.AppendLine($"\t\treturn {Library.LowerFirstCharacter(table.Name)}sByQueryString(querySql: querySql)");
            classText.AppendLine($"\t}}");
        }
        ...
        private string GenerateBindText(string statementParameter, SQLTableColumn column, int columnCounter, bool includeClassNameInParameter)
        {
            ...
            string nullCheckParameterId = column.Nullable ? $"{parameterToBind} ?? NSNull()" : parameterToBind;
            ...
            return column.sqlLiteDataType switch
            {
                ...
                sqlLiteStorageDataTypes.textStore => $"sqlite3_bind_{column.sqlLiteBindType}({statementParameter}, {columnCounter}, NSString(string: {nullCheckParameterId}).utf8String, -1, nil) == SQLITE_OK",
